...
class UjointsInspection extends StatefulWidget {
  final String id;
  final String username;
  const UjointsInspection(
      {super.key, required this.id, required this.username});

  @override
  State<UjointsInspection> createState() => _UjointsInspectionState();
}

class _UjointsInspectionState extends State<UjointsInspection> {
  final List<String> checklistUjoints = [
    S.current.st_column_Ujoint,
    S.current.trans_Ujoint,
    S.current.slipYokeF_Ujoint,
    S.current.slipYokeR_Ujoint,
    S.current.inDif_Ujoint,
    S.current.outDif_Ujoint,
    S.current.pinnion_Ujoint,
    S.current.centerB_Ujoint,
    S.current.center_bearing,
  ];
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ...checklistUjoints.map((label) {
                            return CheckboxListTile(
                              title: Text(label),
                              value: data.getUJointsGlobalCheck(label),
                              onChanged: (v) =>
                                  data.setUJointsGlobalCheck(label, v!),
                        ...
                        ElevatedButton(
                          onPressed: () {
                            ...
                            Navigator.pushReplacement(
                              context,
                              CupertinoPageRoute(
                                builder: (context) => FluidsInspection(
                                    id: widget.id, username: widget.username),
                              ),
                            );
                          },
