...
class InspectionScreen extends StatefulWidget {
  ...
  const InspectionScreen({super.key, required this.id, required this.username});
...
class _InspectionScreenState extends State<InspectionScreen> {
  ...
  final List<String> brakeTypes = [S.current.brake_disc, S.current.brake_drum];
  final List<String> brakeChamberTypes = [
    S.current.types_brakes_chamber_t1,
    S.current.types_brakes_chamber_t2,
  ];
  final List<String> checklistLabels = [
    ...
    S.current.check_wheel_seal,
    S.current.check_wheel_bearings,
    S.current.check_shock_absorber,
  ];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                ...
                children: [
                  ...
                  Padding(
                    padding: const EdgeInsets.symmetric(
                        horizontal: 20, vertical: 10),
                    child: DropdownButtonFormField<String>(
                      value: data.getBrakeType(currentWheel),
                      decoration: InputDecoration(
                        labelText: S.current.types_brakes,
                        border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(10)),
                        filled: true,
                        fillColor: Theme.of(context).cardColor,
                      ...
                      items: brakeTypes
                          ...
                          .toList(),
                      onChanged: (t) => data.setBrakeType(currentWheel, t),
                    ),
                  ),
                  Padding(
                    padding: const EdgeInsets.symmetric(
                        horizontal: 20, vertical: 10),
                    child: DropdownButtonFormField<String>(
                      value: data.getBrakeChamberType(currentWheel),
                      decoration: InputDecoration(
                        labelText: S.current.types_brakes_chamber,
                        border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(10)),
                        filled: true,
                        fillColor: Theme.of(context).cardColor,
                      ),
                      items: brakeChamberTypes
                          .map(
                              (t) => DropdownMenuItem(value: t, child: Text(t)))
                          .toList(),
                      onChanged: (t) =>
                          data.setBrakeChamberType(currentWheel, t),
                    ),
                  ),
                  Padding(
                    padding:
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(S.current.brakes_Checklist,
                              style: TextStyle(
                          ...
                          onPressed: () {
                            ...
                            if (idx < wheels.length - 1) {
                              ...
                              final data = Provider.of<InspectionData>(context,
                                  listen: false);
                              data.setSelectedWheel('Wheel 1');
                              Navigator.pushReplacement(
                                context,
                                CupertinoPageRoute(
                                    builder: (_) => BrakesInspection(
                                        id: widget.id,
                                        username: widget.username)),
                              );
                            }
      ...
      bottomNavigationBar: CustomAppBarBottomNav(
        title: S.current.title_inspection(widget.id),
