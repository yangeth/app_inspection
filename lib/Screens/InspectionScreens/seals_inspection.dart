...
class SealsInspection extends StatefulWidget {
  final String id;
  final String username;
  const SealsInspection({super.key, required this.id, required this.username});

  @override
  State<SealsInspection> createState() => _SealsInspectionState();
}

class _SealsInspectionState extends State<SealsInspection> {
  ...
  final List<String> checklistWheel = [
    S.current.check_wheel_seal,
    S.current.check_wheel_bearings
  ];

  final List<String> checklistSeals = [
    S.current.trans_seal,
    S.current.st_box,
    S.current.rear_d_g,
    S.current.front_d_g,
    S.current.pinion_seal,
    S.current.output_d_s,
    S.current.input_d_s,
    S.current.power_d_g
  ];
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ...checklistSeals.map((label) {
                            return CheckboxListTile(
                              title: Text(label),
                              value: data.getSealsGlobalCheck(label),
                              onChanged: (v) =>
                                  data.setSealsGlobalCheck(label, v!),
                        ...
                        ElevatedButton(
                          onPressed: () {
                            ...
                            Navigator.pushReplacement(
                              context,
                              CupertinoPageRoute(
                                builder: (context) => UjointsInspection(
                                    id: widget.id, username: widget.username),
                              ),
                            );
                          },
