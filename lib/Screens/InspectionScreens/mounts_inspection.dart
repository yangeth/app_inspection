...
class MountsInspection extends StatefulWidget {
  final String id;
  final String username;
  const MountsInspection({super.key, required this.id, required this.username});

  @override
  State<MountsInspection> createState() => _MountsInspectionState();
}

class _MountsInspectionState extends State<MountsInspection> {
  final List<String> checklistMounts = [
    S.current.check_radiator,
    S.current.check_hood_mounts,
    S.current.check_hood_swivel,
    S.current.check_cab_mount,
    S.current.check_sleeper_rod,
    S.current.check_fifth_wheel_bushing,
    S.current.check_front_motor_mounts,
    S.current.check_rear_motor_mounts,
  ];
  int _paginaActual = 2;
                  ...
                  Padding(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            S.current.title_ins_mounts_checklist,
                            style: TextStyle(
                              fontSize: 18,
                          ...
                          ...checklistMounts.map((label) {
                            return CheckboxListTile(
                              title: Text(label),
                              value: data.getMountsGlobalCheck(label),
                              onChanged: (v) =>
                                  data.setMountsGlobalCheck(label, v!),
                        ...
                        ElevatedButton(
                          onPressed: () {
                            ...
                            Navigator.pushReplacement(
                              context,
                              CupertinoPageRoute(
                                builder: (context) => AirBrakesInspection(
                                    id: widget.id, username: widget.username),
                              ),
                            );
                          },
