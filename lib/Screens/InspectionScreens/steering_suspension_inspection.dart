...
class SteeringSuspensionInspection extends StatefulWidget {
  final String id;
  final String username;
  const SteeringSuspensionInspection(
      {super.key, required this.id, required this.username});

  @override
  State<SteeringSuspensionInspection> createState() =>
      _SteeringSuspensionInspectionState();
}

class _SteeringSuspensionInspectionState
    extends State<SteeringSuspensionInspection> {
  final List<String> checklistSteeringAndSuspension = [
    S.current.check_king_ping,
    S.current.check_drag_link,
    S.current.check_upper_torque,
    S.current.check_stabilizer_bar,
    S.current.check_lower_torque,
    S.current.check_front_spring,
    S.current.check_tie_rood,
    S.current.check_rear_spring,
  ];
                  ...
                  Padding(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            S.current.ins_steering_and_suspension_check,
                            style: TextStyle(
                              fontSize: 18,
                          ...
                          ...checklistSteeringAndSuspension.map((label) {
                            return CheckboxListTile(
                              title: Text(label),
                              value: data.getSteeringsGlobalCheck(label),
                              onChanged: (v) =>
                                  data.setSteeringsGlobalCheck(label, v!),
                        ...
                        ElevatedButton(
                          onPressed: () {
                            ...
                            Navigator.pushReplacement(
                              context,
                              CupertinoPageRoute(
                                builder: (context) => MountsInspection(
                                    id: widget.id, username: widget.username),
                              ),
                            );
                          },
